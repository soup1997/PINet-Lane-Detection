...
def Testing():
    ...
    if p.mode == 0:  # check model with test data
        ...
        # out = cv2.VideoWriter('result_video/project_video_result.mp4', fourcc, 11, (512, 256))
        ...
        while cap.isOpened():
            ...
            try:
                if ret is not False:
                    ...
                    frame = region_of_interest(frame) / 255.0
                    frame = np.rollaxis(frame, axis=2, start=0)
                    _, _, ti, steering_angle = test(lane_agent, np.array([frame]))

                    steering_img = cv2.getRotationMatrix2D((512 / 2, 256 / 2), -steering_angle, 0.3)
                    steering_img = cv2.warpAffine(steering_wheel_img, steering_img, (0, 0))
                    curTime = time.time()
                    sec = curTime - prevTime
                    fps = 1 / (sec)
                    s = "FPS : " + str(fps)

                    cv2.putText(ti[0], s, (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0))
                    cv2.putText(steering_img, "Angle: " + str(steering_angle), (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                                (255, 255, 255))
                    cv2.imshow('result', cv2.add(ti[0], steering_img))
                    # out.write(ti[0])
...
def test(lane_agent, test_images, thresh=p.threshold_point):
    ...
    for i in range(num_batch):
        ...
        if len(in_x) >= 3 or len(in_y) >= 3:
            continue
        result_image = util.draw_points(in_x, in_y, deepcopy(image))
        steering_angle = util.get_angle_on_lane(in_x, in_y)
        print("steering_angle: {}".format(steering_angle), end="\n")
        out_x.append(in_x)
        out_y.append(in_y)
        out_images.append(result_image)

    return out_x, out_y, out_images, steering_angle
...
def generate_result(confidance, offsets, instance, thresh):
    ...
    for i in range(len(grid)):
        if (np.sum(feature[i] ** 2)) >= 0:
            ...
            if point_x <= 20 or point_x >= 512 - 20 or point_y <= 170 or point_y >= 240:
                continue
